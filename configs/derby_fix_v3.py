...
echo "1. Clean up - remove the Derby directory entirely"
rm -rf /tmp/hive-conf /tmp/metastore /var/hive/metastore
mkdir -p /tmp/hive-conf

echo "2. Apply config with /tmp/metastore path"
...
echo "3. Init schema (Derby will create /tmp/metastore/)"
cd /tmp
...
r = subprocess.run(
    ['ssh', 'root@192.168.0.211', 'cat > /tmp/derby_fix3.sh'],
...
r2 = subprocess.run(
    ['ssh', 'root@192.168.0.211',
     ...
     'docker exec -i hadoop-arm bash < /tmp/derby_fix3.sh'],
