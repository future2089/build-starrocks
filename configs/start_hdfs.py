...
echo "1. Format NN (if needed)"
if [ ! -f /tmp/hadoop/name/current/VERSION ]; then
  echo "Formatting NN..."
  $HADOOP_HOME/bin/hdfs namenode -format -force 2>&1 | tail -3
else
  echo "NN already formatted"
fi
...
sleep 5
ps aux | grep -E "[N]ame[N]ode|[D]ata[N]ode" | head -5

echo "4. Check ports"
ss -tlnp 2>/dev/null | grep -E "8020|50070|50075" || netstat -tlnp 2>/dev/null | grep -E "8020|50070|50075" || echo "(not listening)"

echo "---DONE---"
"""

r = subprocess.run(
    ['ssh', 'root@192.168.0.211', 'cat > /tmp/start_hdfs.sh'],
...
r2 = subprocess.run(
    ['ssh', 'root@192.168.0.211',
     ...
     'docker exec -i hadoop-arm bash < /tmp/start_hdfs.sh'],
