...
public class TestUGI2 {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("auth_to_local: " + hadoopConf.get("hadoop.security.auth_to_local"));
...
subprocess.run(['docker', 'exec', '-i', 'sr-deploy', 'tee', '/tmp/TestUGI2.java'], input=java, text=True, timeout=10)
r = subprocess.run(['docker', 'exec', 'sr-deploy', 'javac', '-cp', cp, '/tmp/TestUGI2.java', '-d', '/tmp'], capture_output=True, text=True, timeout=30)
if r.returncode != 0:
    ...
    r = subprocess.run(['docker', 'exec', 'sr-deploy', 'java',
        ...
        '-cp', f'/tmp:{cp}',
        'TestUGI2'], capture_output=True, text=True, timeout=30)
    print(r.stdout[:5000])
    if r.stderr:
