...
# KDC database master password - supply via environment, never hardcode.
#   export KDC_MASTER_PASSWORD='...'
...
# Create KDC database
docker exec -e KDC_MASTER_PASSWORD="$KDC_MASTER_PASSWORD" $KDC_CONTAINER \
  bash -c 'printf "%s\n%s\n" "$KDC_MASTER_PASSWORD" "$KDC_MASTER_PASSWORD" | kdb5_util create -s -r ARM.SR.TEST'
